...
class YoloTinyNet(Net):
  ...
  def loss(self, predicts, labels, objects_num):
    ...
    loss = [0, 0, 0, 0]
    print(loss)
    #print("predicts==",predicts.shape.as_list())
    #print("labels==",labels.shape.as_list())
    #print("batch_size==",self.batch_size)
    #print("objects_num==",objects_num.shape.as_list())
    ...
    #tf.add_to_collection:把變量放入一個集合，把很多變量放變成一個列表
    tf.add_to_collection('losses', (loss[0] + loss[1] + loss[2] + loss[3])/self.batch_size)
    #tf.summary.scalar:將訓練過程記錄起來
