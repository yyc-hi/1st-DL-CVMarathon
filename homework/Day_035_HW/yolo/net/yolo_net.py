...
class YoloNet(Net):
  ...
  def __init__(self, common_params, net_params, test=False):
    ...
    #在實際狀況下 train才需要計算loss function
    if not test:
  ...
  def inference(self, images):
    ...
    #有搭配 droupout
    #對 dropout的定義想更了解，可以參考 https://zhuanlan.zhihu.com/p/38200980
  ...
  def body1(self, num, object_num, loss, predict, labels, nilboy):
    ...
    #calculate I tensor [CELL_SIZE, CELL_SIZE, BOXES_PER_CELL]==>1^{object}_{ij}
    #判斷第 I 個網格中第 j 個bbox是否負責這個物體
  ...
  def loss(self, predicts, labels, objects_num):
    ...
    for i in range(self.batch_size):
      ...
      #while_loop(cond, body, [a, n]),[]裡面為body的參數，當符合cond1條件時，則會執行 body1的運算
      #以每一個物件分別計算每一張圖是否有那一個物件，進行損失函數的運算
